...
{
  if (isset($options['min']) && !has_length_greater_than($value, $options['min'] - 1)) : return false;
  elseif (isset($options['max']) && !has_length_less_than($value, $options['max'] + 1)) : return false;
  elseif (isset($options['exact']) && !has_length_exactly($value, $options['exact'])) : return false;
  else : return true;
  endif;
}
...
{
  $email = User::find_by_email($client_email);
...
{
  $client = User::find_by_phone($phone);
